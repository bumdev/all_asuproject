...
{
    public partial class InstallJournal : ULPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                /*if (Request["id"] != null)
                {
                    ...
                    window1.NavigateUrl = "check/InstallDeviceDet.aspx?id=" + Request["id"];// "FabonentDet.aspx?id=" + Request["id"];
        ...
        protected void radgridDevice_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            ...
            {
                radgrid.DataSource = (DataView)dsInstallJournal.Select(DataSourceSelectArguments.Empty);
            }
        }
        protected void radgridDevice_ItemCommand(object sender, GridCommandEventArgs e)
        {
            if (e.CommandName == "Requests")
            {
                if (e.Item is GridDataItem)
                {
                    ...
                    InstallDet1.OrderID = Utilities.ConvertToInt(itemValue);  // Convert.ToInt32(e.Item.OwnerTableView.Items[e.]);
                    InstallDet1.Bind();
                }
            }
        }
