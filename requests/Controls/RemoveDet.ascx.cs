...
{
    public partial class RemoveDet : ULControl
    {
        ...
        protected void radbutArchive_OnClick(object sender, EventArgs e)
        {
            RequestsDO fado = new RequestsDO();
            fado.RemoveToArchive(Utilities.ConvertToInt(hfODID.Value), GetCurrentUser().ID);
            _OrderID = Utilities.ConvertToInt(hfODID.Value);
            Bind();
        }
    }
